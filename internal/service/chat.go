...
func (api *ChatService) GetOrCreateChatWithUser(ctx context.Context, selfUserID int, userID int) (int, error) {
	...
	if err != nil {
		FromContext(ctx).Error("Failed to check user existence", zap.Error(err))
		return 0, domain.ErrDB
	}
	if !isUserExist {
		FromContext(ctx).Warn("User not found")
		return 0, domain.ErrNotExist
	}
	if userID == selfUserID {
		FromContext(ctx).Warn("Failed to create or get chat with same self user")
		return 0, domain.ErrInvalidInput
	}
	chatID, err := api.chatStore.GetOrCreateChatWithUser(ctx, selfUserID, userID)
	if err != nil {
		FromContext(ctx).Error("Failed to create or get chat with user", zap.Error(err))
		return 0, domain.ErrDB
	}

	FromContext(ctx).Info("Chat created or retrieved", zap.Int("chatID", chatID), zap.Int("chatWithUserID", userID))
	return chatID, nil
}
...
func (api *ChatService) GetMessagesByChatId(ctx context.Context, params domain.PaginateQueryParams, userID int, chatID int) (*MessagesWithAuthors, error) {
	...
	if err != nil {
		FromContext(ctx).Error("Failed to check membership", zap.Error(err), zap.Int("chatID", chatID))
		return nil, domain.ErrDB
	}
	if !isMember {
		FromContext(ctx).Warn("User not a member of chat", zap.Int("chatID", chatID))
		return nil, domain.ErrAccessDenied
	}

	messages, err := api.messageStore.GetMessagesByChatId(ctx, chatID, params.Limit, params.Offset)
	if err != nil {
		FromContext(ctx).Error("Failed to get messages", zap.Error(err), zap.Int("chatID", chatID))
		return nil, domain.ErrDB
	}
	...
	if err != nil {
		FromContext(ctx).Error("Failed to get authors", zap.Error(err), zap.Ints("authorIDs", authorIDs))
		return nil, domain.ErrDB
	}
	...
	FromContext(ctx).Info("Messages retrieved successfully", zap.Int("chatID", chatID), zap.Int("limit", params.Limit), zap.Int("offset", params.Offset))
...
func (api *ChatService) CreateMessage(ctx context.Context, userID int, chatID int, message domain.Message) (int, error) {
	...
	if err != nil {
		FromContext(ctx).Error("Failed to get chat", zap.Error(err), zap.Int("chatID", chatID))
		return 0, domain.ErrDB
	}

	if !exits {
		FromContext(ctx).Warn("Chat not found", zap.Int("chatID", chatID))
		return 0, domain.ErrNotExist
	}
	...
	if err != nil {
		FromContext(ctx).Error("Failed to create message", zap.Error(err), zap.Int("chatID", chatID))
		return 0, domain.ErrDB
	}
	...
	go func(ctx context.Context, userId int, chatID int) {
		...
		if err != nil {
			FromContext(ctx).Error("Fail to get chat", zap.Error(err))
			return
		}
		recipients, err := api.chatStore.GetOtherChatMembersIdByAuthorId(ctx, userID, chatID)
		if err != nil {
			FromContext(ctx).Error("Fail to get recipients", zap.Error(err))
			return
		}
		data, err := json.Marshal(chat)
		if err != nil {
			FromContext(ctx).Error("Fail to marshal chat", zap.Error(err))
			return
		}
	...
	FromContext(ctx).Info("Message created successfully", zap.Int("messageID", messageID), zap.Int("chatID", chatID))
...
func (api *ChatService) GetUserChats(ctx context.Context, userID int, params domain.PaginateQueryParams) ([]domain.FullChat, error) {
	...
	if err != nil {
		FromContext(ctx).Error("Failed to get chats", zap.Error(err))
		return nil, domain.ErrDB
	}

	FromContext(ctx).Info("Chats retrieved successfully", zap.Int("limit", params.Limit), zap.Int("offset", params.Offset))
	return chats, nil
}
