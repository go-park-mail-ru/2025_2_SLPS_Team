...
func WaitForGRPC[T any](target string, constructor func(grpc.ClientConnInterface) T, retries int, delay time.Duration) (T, *grpc.ClientConn, error) {
	...
	for i := 0; i < retries; i++ {
		...
		defer cancel()
		conn, err := grpc.DialContext(ctx, target, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithUnaryInterceptor(ClientUnaryInterceptor()), grpc.WithBlock())
		if err == nil {
