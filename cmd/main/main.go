...
func NewApiRouter(logger *zap.Logger,
	...
	commentRouter.Use(auth.AuthMiddleware)
	commentRouter.HandleFunc("", commentHandler.CreateComment).Methods("POST", "OPTIONS")
	commentRouter.HandleFunc("/{id:[0-9]+}", commentHandler.GetComment).Methods("GET")
	commentRouter.HandleFunc("/{id:[0-9]+}", commentHandler.UpdateComment).Methods("PUT", "OPTIONS")
